# Golden-path helpers. Run from the repo root.
...
SERVICE_DIR  := services/example-api
TF_DIRS      := modules/ecs-api modules/platform \
	platform/envs/dev platform/envs/staging platform/envs/prod \
	services/example-api/envs/dev services/example-api/envs/staging services/example-api/envs/prod
...
help:
	@echo "docker-build     Build the example-api image locally"
	@echo "docker-run       Run the image on :8080 (SERVICE_NAME, VERSION from TAG)"
	@echo "tf-fmt           terraform fmt -recursive"
	@echo "tf-validate      terraform init -backend=false && validate in each root/module"
	...
	@echo "new-service      Scaffold services/$$NAME from example-api"
	@echo "inception        From-nothing: scaffold + image + register pipelines (NAME=)"
...
# NAME=orders-api make new-service
new-service:
	@test -n "$(NAME)" || (echo "NAME is required, e.g. make new-service NAME=orders-api" >&2; exit 1)
